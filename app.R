...
# Over time plots ---------------------------------
  ...
  output$ot_cases <- renderPlotly({
    ...
    ggplotly({temp <- ggplot(data_over_time(), aes(dateRep, if(!input$ot_per100k){cases}else{cases/(popData2019/10^5)}, group = countriesAndTerritories, color = countriesAndTerritories, text=paste("Date:", dateRep, "<br>Country:", countriesAndTerritories, "<br>New cases:", cases, "<br>Cumulative cases:", format(cumulative_cases, big.mark=",", trim=TRUE)) )) +
               geom_line() + 
               geom_point(size = 0.5) +
  ...
  output$ot_deaths <- renderPlotly({
    ...
    ggplotly({temp <- ggplot(data_over_time(), aes(dateRep, if(!input$ot_per100k){deaths}else{deaths/(popData2019/10^5)}, group = countriesAndTerritories, color = countriesAndTerritories, text=paste0("Date: ", dateRep, "<br>Country: ", countriesAndTerritories, "<br>Deaths: ", deaths, "<br>Cumulative deaths: ", cumulative_deaths) )) +
               geom_line() + 
               geom_point(size = 0.5) +
